...
def sfm_compute_depthmaps(current_path, opensfm_config, self_compute=False, self_path=''):
    import open_compute_depthmaps
    try:
        if self_compute:
            
            open_compute_depthmaps.open_compute_depthmaps(current_path, opensfm_config, self_compute, self_path)
        else:
            open_compute_depthmaps.open_compute_depthmaps(current_path, opensfm_config)
    except Exception as e:
        print(traceback.print_exc())
        return False
                    ...
                    if(taskName == 'exif'):
                        # check 
                            ...
                            if (is_complete):
                                ...
                                write_json({'exiftime': end_timer}, 'exiftime.json')
                        ...
                        if(is_complete):
                            ...
                            #write time into json file timer
                            end_timer = timer()-start_timer
                            write_json({'detect_features': end_timer}, 'dfeat.json')
                        ...
                        #write time into json file timer
                        end_timer = timer()-start_timer
                        #write_json_({req_node_path+'feature_matching': end_timer}, req_node_path_timer)
                        ...
                        image_list = list(request.compute_filenames)

                        #print('pairs ' + str(pair1) + ' ' + str(pair2))

                        pairs_matches = sfm_feature_matching(req_node_path, image_list , image_list, opensfm_config)
