        ...
        try:
            ...
            if task['clientid'] == task['nodeid']:
                ...
                if task['title'] == 'extract_exif':
                    ...
                    matches = lib.sfm_feature_matching(task['filepath'],unique_id,unique_id,task['opensfm_config'])
                ...
                if task['title'] == 'upload_image':
                    ...
                    #response = task['client'].upload(task['filepath'], [feature_0])
                    ...
                    response = task['client'].sendTask('feature_matching_pairs', task['nodeid'], '/matches', unique_id)
    ...
    try:
        ...
        for number in range(len(nodeid_list)):
            ...
            if nodeid_list[number] == nodeid:
                job_task = {'title': 'feature_matching_pairs', 'nodeid': 1,  'filepath': file_path, 'results': results, 'lock': thread_lock, 'clientid': node_client_num, 'opensfm_config': opensfm_config, 'pairs': n}
            else:
                job_task = {'title': 'feature_matching_pairs', 'client': node_client[nodeid_list[number]], 'nodeid': 1,  'filepath': file_path, 'results': results, 'lock': thread_lock, 'clientid': nodeid_list[number], 'pairs':n}
                node_client_num = number + 1  
            job_queue.put(job_task)
            current = end
            if number+1 != len(nodeid_list)-1:
                end =  (len(pairs)/(len(nodeid_list))) * (1+number) 
                print(end)
            else:
                end = len(pairs)
        ...
        job_queue.join()
        print('end of the queue range')

        print(len(results))


        print('end of the finished')



        #     if node_client_num == nodeid:
        #         job_task = {'title': 'feature_matching_pairs', 'nodeid': 1, 'pair': each, 'filepath': file_path, 'results': results, 'lock': thread_lock, 'clientid': node_client_num, 'opensfm_config': opensfm_config, 'pairs'}
        #     else:
